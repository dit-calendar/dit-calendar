...
createUser :: (DBRepo.MonadDBUser m, HasAcidState m UserAcid.UserList) => String -> m User
...
deleteUser :: (DBRepo.MonadDBUser m, HasAcidState m UserAcid.UserList) =>
            User -> m ()
deleteUser user = DBRepo.delete $ UserAcid.DeleteUser (Data.Domain.User.userId user)

updateUser :: (DBRepo.MonadDBUser m, HasAcidState m UserAcid.UserList) => User -> m ()
updateUser user = DBRepo.update $ UserAcid.UpdateUser user

updateName :: (DBRepo.MonadDBUser m, HasAcidState m UserAcid.UserList) =>
            User -> String -> m ()
updateName user newName = updateUser user {name = newName}

addCalendarEntryToUser :: (DBRepo.MonadDBUser m, HasAcidState m UserAcid.UserList) =>
                        User -> EntryId -> m ()
addCalendarEntryToUser user entryId =
    updateUser user {calendarEntries = calendarEntries user ++ [entryId]}

deleteCalendarEntryFromUser :: (DBRepo.MonadDBUser m, HasAcidState m UserAcid.UserList) =>
                            User -> EntryId -> m ()
deleteCalendarEntryFromUser user entryId =
    updateUser user {calendarEntries = delete entryId (calendarEntries user)}

addTaskToUser :: (DBRepo.MonadDBUser m, HasAcidState m UserAcid.UserList) =>
                TaskId -> User -> m ()
addTaskToUser taskId user =
    updateUser user {belongingTasks = belongingTasks user ++ [taskId]}

deleteTaskFromUser :: (DBRepo.MonadDBUser m, HasAcidState m UserAcid.UserList) =>
                    TaskId -> User -> m ()
