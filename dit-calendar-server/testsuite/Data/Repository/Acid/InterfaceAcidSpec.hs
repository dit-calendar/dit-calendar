...
withDatabaseConnection = initDatabase def{ CalendarEntry.description="Foo", CalendarEntry.entryId=0, owner=0,
        ...
        context "CalendarEntry" $ do
          describe "create" $ do
              it "new and check existence" $
                \c -> do
                  let newCalendarEntry = def { CalendarEntry.description="Zahnarzt", CalendarEntry.entryId=0, owner=0,
                    startDate=dbDate, endDate=dbDate }
                  ...
                  fromJust findNewCalendarEntryFromDB `shouldBe` def{ description="Zahnarzt", entryId=newCalendarEntryId, owner=0,
                        startDate=dbDate, endDate=dbDate}
              ...
              it "new and check nextEntryId" $
                \c -> do
                  ...
                  let calendarEntry = def { CalendarEntry.description="Zahnarzt", CalendarEntry.entryId=0, owner=1,
                    startDate=dbDate, endDate=dbDate }
                  ...
                  let newCalendarEntry = def { CalendarEntry.description="Zahnarzt2", CalendarEntry.entryId=0, owner=1,
                  ...
                  fromJust entryState `shouldBe` def{ description="Zahnarzt2", entryId=oldId + 1, owner=1,
                        startDate=newDate2, endDate=newDate2}
          ...
          describe "find" $
              it "by EntryId" $
                \c -> do
                  ...
                  entryState `shouldSatisfy` isJust
                  fromJust entryState `shouldBe` def{ description="Foo", entryId=0, owner=0,
                      startDate=dbDate, endDate=dbDate }

          describe "delete" $
              it "create/delete Entry and check existence" $
                \ c -> do
                  let newCalendarEntry = def { CalendarEntry.description="Zahnarzt", CalendarEntry.entryId=0, owner=1,
                    startDate=dbDate, endDate=dbDate }
                  newCalendarEntryFromDB <- update c (NewEntry newCalendarEntry)
                  let newCalendarEntryId = CalendarEntry.entryId newCalendarEntryFromDB
          ...
          describe "update" $ do
              it "change attributes and check changes" $
                \c -> do
                  --erstelle Objekt
                  let newCalendarEntry = def { CalendarEntry.description="Termin 1", CalendarEntry.entryId=0, owner=2,
                    startDate=dbDate, endDate=dbDate }
                  newCalendarEntryFromDB      <- update c (NewEntry newCalendarEntry)
                  let newCalendarEntryId = CalendarEntry.entryId newCalendarEntryFromDB
                  -- update Objekt
                  ...
                  entryState                  <- query c $ CalendarEntryAcid.EntryById newCalendarEntryId
                  entryState `shouldSatisfy` isJust
                  fromJust entryState `shouldBe` def { description="Termin 2", entryId=newCalendarEntryId, owner=2, version=1,
                    startDate=dbDate, endDate=dbDate}
              it "change data and check version" $
                \ c -> do
                    ...
                    entryState                  <- query c $ CalendarEntryAcid.EntryById 0
                    fromJust entryState `shouldBe` def { description="Foo", entryId=0, owner=0, version=1,
                        startDate=dbDate, endDate=dbDate}

                    let updatedEntry = (fromJust findCalendarEntryFromDB) {version = 1}
                    _                           <- update c $ CalendarEntryAcid.UpdateEntry updatedEntry
                    entryState                  <- query c $ CalendarEntryAcid.EntryById 0
                    fromJust entryState `shouldBe` def { description="Foo", entryId=0, owner=0, version=2,
                        startDate=dbDate, endDate=dbDate}
